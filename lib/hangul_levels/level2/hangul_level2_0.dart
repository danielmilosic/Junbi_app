...
class HangulLevel20 extends StatelessWidget {
  const HangulLevel20({Key? key}) : super(key: key);
...
class _HangulContentState extends State<_HangulContent> {
  ...
  // Subset indexes for simpler keyboard
  final List<int> _uiInitialIndexes = [0, 2, 3, 5, 6, 7, 9, 11]; // ㄱ ㄴ ㄷ ㄹ ㅁ ㅂ ㅅ ㅇ
  final List<int> _uiVowelIndexes = [0, 4, 8, 13, 18, 20]; // ㅏ ㅓ ㅗ ㅜ ㅠ ㅣ
  ...
  setState(() {
    _textColor = (preview == '산' || preview == '심사' || preview == '다섯')
        ? Colors.green
        : Colors.white;
      ...
      child: Column(
        ...
        LinearProgressIndicator(
          ...
          const SizedBox(height: 6),
                        ...
                        child:_JamoGrid(
            items: [
              _Jamo('시', 'SHi', 'wie in "Schiff"'),
              _Jamo('사', 'Sa', 'stimmlos, wie in "hauSArbeit", nicht etwa stimmhaft wie in "SAnd"'),
              _Jamo('웃', 'ut', 'wie in "gut"'),
            ],
          ),),
...
SingleChildScrollView(
  ...
  child: Row(
    children: [
      _buildAudioCard(context, '산 - Berg', 'audio/hangul/san.mp3'),
      _buildAudioCard(context, '심사 - Prüfung', 'audio/hangul/shimsa.mp3'),
      _buildAudioCard(context, '다섯 - Fünf', 'audio/hangul/daseot.mp3'),
    ],
  ),
),
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    ...
                    Navigator.pushAndRemoveUntil(
                      context,
                      MaterialPageRoute(builder: (_) => HangulLevel21()), // or MainPage()
                      (route) => false, // remove all previous routes
                    );
                  },
                ),
